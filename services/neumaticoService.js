...
const neumaticoService = {
    ...
    obtenerTodos: async (filtroSupervisor = null) => {
        let sql = `
            SELECT 
                ...
                C.DISEÑO,
                C.PR_LONAS AS PR,
                C.INDICE_CARGA AS CARGA,
                C.INDICE_VELOCIDAD AS VELOCIDAD,
                C.DOT_FABRICACION AS FECHA_FABRICACION_COD, 
                C.DOT_FABRICACION AS FECHA_FABRICACIO, -- Legacy alias duplicate
                C.RQ,
                C.OC,
                TRIM(C.PROYECTO) AS PROYECTO,
                C.COSTO_INICIAL AS COSTO,
                C.PROVEEDOR_NOMBRE AS PROVEEDOR,
                C.FECHA_COMPRA,
                C.FECHA_REGISTRO,  -- NULL si no tiene valor
                TRIM(C.SUPERVISOR_ACTUAL) AS USUARIO_SUPER,
                
                -- Estado y Ubicación
                C.PORCENTAJE_VIDA AS ESTADO,          -- Porcentaje (Barra)
                C.ESTADO_ACTUAL,                       -- Estado actual del neumático (ID de NEU_ESTADO)
                E.DESCRIPCION AS TIPO_MOVIMIENTO,     -- "DISPONIBLE", "ASIGNADO" (Desde Catálogo)
                TRIM(C.PLACA_ACTUAL) AS PLACA,
                C.POSICION_ACTUAL AS POSICION_NEU,
                
                -- Mediciones
                C.REMANENTE_ACTUAL AS REMANENTE,
                C.PRESION_ACTUAL AS PRESION_AIRE,
                C.TORQUE_ACTUAL,
                C.KM_TOTAL_VIDA AS KILOMETRO,
                
                -- Campo calculado para filtros legacy (BAJA_DEFINITIVA, RECUPERADO, ACTIVO)
                -- Campo calculado para filtros legacy (BAJA_DEFINITIVA, RECUPERADO, ACTIVO)
                CASE 
                    WHEN E.CODIGO_INTERNO = 'BAJA' THEN 'BAJA_DEFINITIVA'
                    WHEN E.CODIGO_INTERNO = 'RECUPERADO' THEN 'RECUPERADO'
                    ELSE 'ACTIVO'
                END AS ESTADO_NEUMATICO
        ...
        const params = [];
        if (filtroSupervisor) {
            // Mostrar mis neumáticos ASIGNADOS o los de DISPONIBLE que no tienen dueño (Globales)
            // USAMOS LIKE PARA EVITAR PROBLEMAS DE PADDING EN CAMPOS CHAR DB2
            // Nota: E.CODIGO_INTERNO viene de NEU_ESTADO, pero también verificamos C.ESTADO_ACTUAL
            sql += " WHERE (UPPER(C.SUPERVISOR_ACTUAL) LIKE UPPER(?) OR (E.CODIGO_INTERNO = 'DISPONIBLE' AND C.SUPERVISOR_ACTUAL IS NULL))";
            params.push(filtroSupervisor.trim() + '%');
        }

        sql += ' ORDER BY C.CODIGO_CASCO';
    ...
    registrarMovimiento: async ({
        idNeumatico, codigo, tipoAccion, idVehiculo, placa,
        posicionAnterior, posicionNueva, odometro, remanente,
        presion, torque, observacion, usuario, estadoDestino, // <--- Agregado torque
        kmRecorrido, kmVida // <--- Explicitly Destructured
    }) => {
        ...
        const sql = `
            INSERT INTO SPEED400AT.NEU_DETALLE (
                ID_NEUMATICO, CODIGO_CASCO, ID_VEHICULO, 
                PLACA, POSICION_ANTERIOR, POSICION_NUEVA, ODOMETRO_VEHICULO,
                REMANENTE_MEDIDO, PRESION_MEDIDA, TORQUE_APLICADO, OBSERVACION, USUARIO_REGISTRO,
                ID_ESTADO, ID_ACCION, TIPO_ACCION, 
                KM_RECORRIDOS_ETAPA
                -- KM_TOTAL_VIDA (Not present in Detail table)
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;

        const params = [
            idNeumatico, codigo, idVehiculo || null,
            placa || null, posicionAnterior || null, posicionNueva || null, odometro || null,
            remanente || null, presion || null, torque || null, observacion || '', usuario || 'SISTEMA',
            idEstadoResolved,
            idAccionResolved,
            tipoAccion,
            kmRecorrido || 0
    ...
    asignarNeumatico: async (data, usuario) => {
        const {
            ...
            Placa, Posicion, Odometro
        } = data;

        // Validar que PresionAire sea obligatorio
        if (!PresionAire) {
            throw new Error('La presión de aire es obligatoria para asignar un neumático');
        }

        // 1. Obtener ID y ESTADO ACTUAL
        const sqlGetId = `
            SELECT C.ID_NEUMATICO, E.CODIGO_INTERNO as ESTADO_CODIGO 
            FROM SPEED400AT.NEU_CABECERA C
            LEFT JOIN SPEED400AT.NEU_ESTADO E ON C.ID_ESTADO = E.ID_ESTADO
        ...
        const sqlUpdate = `
            UPDATE SPEED400AT.NEU_CABECERA SET
                ID_ESTADO = (SELECT ID_ESTADO FROM SPEED400AT.NEU_ESTADO WHERE CODIGO_INTERNO = 'ASIGNADO'),
                PLACA_ACTUAL = ?,
                POSICION_ACTUAL = ?,
                PRESION_ACTUAL = ?,
                REMANENTE_ACTUAL = ?,
        ...
        await db.query(sqlUpdate, [
            Placa, Posicion, PresionAire, Remanente, TorqueAplicado, Odometro, idNeumatico
        ]);
    ...
    registrarInspeccion: async (data, usuario) => {
        const {
            CODIGO, REMANENTE, PRESION, KILOMETRO, OBSERVACION, PLACA
        } = data;
        ...
        const sqlGet = `
            SELECT C.ID_NEUMATICO, C.REMANENTE_INICIAL, C.REMANENTE_ACTUAL, 
                   C.ODOMETRO_AL_MONTAR, C.PLACA_ACTUAL, C.POSICION_ACTUAL,
        ...
        `;
        const result = await db.query(sqlGet, [CODIGO]);
        ...
        if (historial && historial.length > 0) {
            const kmHist = historial[0].KILOMETRO;
        ...
        const sqlUpdate = `
            ...
            WHERE ID_NEUMATICO = ?
        `;
        const resUpdate = await db.query(sqlUpdate, [REMANENTE, PRESION, nuevoTotalVida, nuevoPorcentaje, neumatico.ID_NEUMATICO]);
        console.log(`[registrarInspeccion] Cabecera Actualizada ID ${neumatico.ID_NEUMATICO}: Porcentaje: ${nuevoPorcentaje}%`);
    ...
    obtenerUltimosMovimientos: async (codigo, usuario_super = null) => {
        ...
        let query = `
            SELECT 
                ...
                C.CODIGO_CASCO AS CODIGO,
                C.ODOMETRO_AL_MONTAR AS ODOMETRO_INICIAL,
                C.DISEÑO,
                C.MEDIDA,
                M.MARCA,
        ...
        query += ` ORDER BY d.FECHA_SUCESO DESC`;
