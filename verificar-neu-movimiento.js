...
async function verificarNeuMovimiento() {
  try {
    ...
    const count19 = await db.query("SELECT COUNT(*) as total FROM SPEED400AT.NEU_MOVIMIENTO WHERE CODIGO = '3000019'");
    console.log(`3000019: ${count19[0].total} registros total`);
    
    const count21 = await db.query("SELECT COUNT(*) as total FROM SPEED400AT.NEU_MOVIMIENTO WHERE CODIGO = '3000021'");
    ...
    const ultimo19 = await db.query(
      "SELECT TOP 1 CODIGO, POSICION_NEU, TIPO_MOVIMIENTO, FECHA_MOVIMIENTO FROM SPEED400AT.NEU_MOVIMIENTO WHERE CODIGO = '3000019' ORDER BY ID_MOVIMIENTO DESC"
    );
    ...
    const ultimo21 = await db.query(
      "SELECT TOP 1 CODIGO, POSICION_NEU, TIPO_MOVIMIENTO, FECHA_MOVIMIENTO FROM SPEED400AT.NEU_MOVIMIENTO WHERE CODIGO = '3000021' ORDER BY ID_MOVIMIENTO DESC"
    );
